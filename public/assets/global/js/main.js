$(document).ready(function () {
    ...
    // Enable Editors
    if($('.wk-editor-textarea').length > 0){
        var editor = new Simditor({
            textarea: $('.wk-editor-textarea'),
            toolbar: ['title', 'bold', 'italic', 'underline', 'strikethrough', 'fontScale', 'color', 'ol', 'ul', 'blockquote', 'hr', 'indent', 'outdent', 'alignment']
            //optional options
        });
        $('.wk-editor-textarea').hide();
    }

    //Set Background Color Black if wrapper's height is bigger than window.innerHeight
    if($('#wrapper').height() > window.innerHeight)
        $('body').css('background-color', '#222d32');
